...
from dataset_ext import CreativeDataset
from model_ext import CreativeGeneratorModel
from swin_emb_model import SwinModel as SwinEmbModel
from transformers import DistilBertTokenizer, DistilBertModel
from bert_emb_model import NLPBertModel as BertEmbModel_v0
from bert_emb_model_ctr import NLPBertModel as BertEmbModel
...
def main(param):
    ...
    if param.load_pretrain:  # 如果要Load Pretrain模型，则把使用过商品图片和标题预训练过的Swin和Bert进行加载
        ...
        if os.path.exists(param.bert_emb_checkpoint_path):
            print("load stat from bert-emb", param.bert_emb_checkpoint_path)
            if not os.path.exists(param.bert_emb_meta_path):
                print("[Erorr] no input bert emb meta path")
                sys.exit(-1)
            bert_emb_model = BertEmbModel(hash_path=param.bert_emb_meta_path, bert_model=bert_model, device=device, model_flag="distilbert")
            bert_emb_model.load_state_dict(torch.load(param.bert_emb_checkpoint_path)['model_state'])
            model.bert_model = bert_emb_model.bert
            print("load bert-emb complete")
    else:
...
def extract_feature(model, test_dataset, test_dataloader, device, epoch, param):
    ...
    save_folder_txt = param.save_folder_txt
    if not os.path.exists(save_folder_img):
        os.mkdir(save_folder_img)
    if not os.path.exists(save_folder_txt):
        os.mkdir(save_folder_txt)
    model.eval()
    tmp_pidx = 0
    with torch.no_grad():
        for step, batch in enumerate(tqdm(test_dataloader)):
            ...
            txt_fea = model.extract_txt(batch)  # bs, txt_dim=768
            
            bs = txt_fea.shape[0]
            img_fea = img_fea.reshape(bs, -1, img_fea.shape[-1])   # bs, #creatives, img_dim
            img_fea_list = img_fea.tolist()
            txt_fea_list = txt_fea.tolist()
            
            # get itemid
            valid_list = batch['valid']  # bs, #creatives
            for tidx in range(txt_fea.shape[0]):
                itemid = test_dataset.samples[tmp_pidx][0]['item_id']
                cre_valid_list = valid_list[tidx]   # #creatives
                img_flist = img_fea_list[tidx]  #  #creatives, img_dim
                txt_f = txt_fea_list[tidx]      # txt_dim
                write_path = os.path.join(save_folder_txt, f"{itemid}.pkl")
                with open(write_path, 'wb') as fw:
                    pickle.dump(txt_f, fw)
...
if __name__ == '__main__':
    ...
    param.add_argument("--save-folder-txt", type=str,
                       default=r"", help="save txt feature folder")
